          ...
          <div className="flex overflow-hidden w-max">
            <ul className="flex items-center gap-8 scroller__inner">
              {partners.map((partner, index) => (
                <li key={index} className="flex items-center justify-between gap-4 px-4 py-2">
                  <a href={partner.link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4">
                    <Image src={partner.img} alt={partner.name} width={40} height={40} className="w-[40px] h-[40px]" />
                    <span className="text-[24px] font-semibold">{partner.name}</span>
                  </a>
                </li>
              ))}
            </ul>
            ...
            <ul className="flex items-center gap-8 ml-6">
              {partners.map((partner, index) => (
                <li key={`duplicate-${index}`} className="flex items-center justify-between gap-4 px-4 py-2">
                  <a href={partner.link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4">
                    <Image src={partner.img} alt={partner.name} width={48} height={48} className="w-12 h-12 rounded-full" />
