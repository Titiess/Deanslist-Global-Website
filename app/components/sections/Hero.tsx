import { CircleArrowUp, ArrowDown, ArrowRight } from "lucide-react";
...
function Hero() {
  return (
    <div id="hero" className="relative pt-20 px-4 md:text-center flex flex-col md:items-center">
      ...
      {/* Badge Section */}
      <div className="flex justify-center items-center space-x-4 mb-6 md:mt-5 bg-[#9A59BA80] p-1 rounded-full cursor-pointer text-xs md:text-base max-w-fit">
        <span className="bg-white text-[#9A59BA] md:text-[15px] py-1 px-3 rounded-full">
          New feature
        </span>
        <p className="flex gap-2 items-center text-[12px] md:text-[15px]">
      ...
      <h1 className="text-4xl md:text-[60px] md:leading-[70px] font-bold mb-8">
        <p>
          Web3 Native <span className="italic font-light font-editorial-new">Network State,</span>
        </p>
	  ...
	  <Link href="https://discord.gg/deanslist" target="blank"  rel="noopener noreferrer">
            ...
            Get Started
				...
				<Link href="https://github.com/dean-s-list" target="blank"  rel="noopener noreferrer">	
				<button className="text-white hidden py-2  px-6 rounded-lg font-medium hover:bg-white hover:text-purple-900 transition md:flex gap-2 items-center mt-1">
      ...
      <div className="w-full hidden md:flex justify-center mt-[76px] overflow-hidden h-[320px] relative">
        ...
        <div className="w-[1024px] h-[1024px] rounded-full translate-y-4">
          <Image src="/continents.png" alt="Globe" width={1024} height={1024} className="w-full" />
        </div>
      </div>
    </div>
  );
